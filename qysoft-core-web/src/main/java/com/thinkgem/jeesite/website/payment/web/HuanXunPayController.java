...
import com.thinkgem.jeesite.modules.user.service.PayMessService;
import com.thinkgem.jeesite.modules.user.service.PayOrderService;
...
public class HuanXunPayController extends BaseController {
    ...
    @Autowired
    private PayMessService payMessService;
    ...
    private String s2sNotify(String resultXml, String merCode, String directStr, String ipsRsaPub) {
        ...
        if (status.equals("Y")) {   //交易成功
            ...
            //判断金额、报文入库
            PayOrder payOrder = payOrderService.getByOrderNo(merBillNum);
            if(payOrder==null){
                return RETURN_FAIL;
            }
            if(!payOrder.getMoney().equals(getAmount(resultXml))){
                return RETURN_FAIL;
            }
            PayMess payMess = new PayMess();
            payMess.setBillNo(getIpsBillNo(resultXml));
            payMess.setOrderNo(merBillNum);
            payMess.setPayType("1");
            payMess.setMessage(resultXml);
            payMessService.save(payMess);
